...
回顾前面说过$\widetilde{GH}(\mathbb{G})$不是扭结不变量，但除掉$2^{n-1}$后是，这实际上是在说$\widehat{GH}(\mathbb{G})$才是扭结不变量，更近一步，这里结构更复杂的$GC^-(\mathbb{G})$也是扭结不变量（和格点选取无关但是平凡结的格点表示对结果有影响）。

格同调作为（symmetrized）Alexander多项式的范畴化可以从下面的等式一窥：

$$
\chi(\widetilde{GH}(\mathbb{G}))=(1-t^{-1})^{n-1}\cdot\Delta_K(t)
$$

这里$n$是$\mathbb{G}$格点表示的格点大小，前面说过$\widetilde{GH}$是和格点表示有关的。以及：

$$
\chi(\widehat{GH}(K))=\Delta_{K}(t)
$$

其中对于任意的双分级向量空间$X=\bigoplus\_{d,s}X\_{d,s}$定义其分级欧拉示性类为：
