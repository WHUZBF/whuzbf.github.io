...
这里我们色指标用了$a$和$\tilde{a}$两种不同的记号，表示我们考虑最一般的情况，也就是标量场带两个色指标$\Phi^{I,J}$，处于$\mathrm{U}(N)\times\mathrm{U}(\tilde{N})$的自伴表示（所以很多时候也叫**bi-adjoint $\Phi^3$ 理论**）。可以证明下面的（推广的）$\Phi^3$理论的振幅确实由上面的CHY形式描述：
...
总的振幅就是把上面的振幅乘上double Trace，但是实际上更实用的是用下面的**色基**去写：
...
这里（包括下面的展开式）$\alpha\in S_{n-2}$，因为模掉轮换相当于固定两个外腿不参与置换，这里我们选择固定$1,n$：
...
Yang-Mills-Scalar理论是标量场和YM场的耦合，标量场带有色指标，处于$SU(N)$群的某个表示，还带有味指标表示可能又多种标量场耦合。YM场胶子处于$SU(N)$群的自伴表示之中，拉格朗日量如下：
...
\begin{equation}
\label{compact}
K_a\cdot K_b=k_a\cdot k_b,\quad K_a\cdot\mathcal{E}_b=\begin{cases}k_a\cdot\epsilon_b,&\mathcal{E}_b\text{ external},\\0,&\mathcal{E}_b\text{ internal},\end{cases}\mathcal{E}_a\cdot\mathcal{E}_b=\begin{cases}\epsilon_a\cdot\epsilon_b,&\text{both external},\\\delta^{I_a,I_b},&\text{both internal},\\0,&\text{else}.\end{cases}
\end{equation}
$$
...
这里用虚线隔开，表明了每个分块小矩阵的指标变化范围，是$g$的部分还是$s$的部分。注意由于$K_a\cdot K_b=k_a\cdot k_b$，所以左上角的$A_n$部分不会变，剩下的都可以从\eqref{compact}看出来。显然这个分块对焦矩阵导致：
...
就好了，而且$g$和$s$都可以是空集，不难验证，$g=\varnothing$时理论被积函数回到纯标量振幅。$s=\varnothing$时理论回到纯YM振幅。
...
这部分的核心就是在找不同的满足$SL(2,\mathbb{C})$协变性和置换不变性的CHY被积函数Block。构造标量场振幅，或者YMS理论的标量振幅部分的时候我们确实是这样，构造新的Block，比如$A,X,\mathcal{X},\cdots$。后面构造带胶子引力子散射振幅的时候，我们不是构造新的Block，而是引入了，compactify，squeeze 和 generalized compactify，对矢量粒子CHY被积函数的重要组成$\operatorname{Pf}'\Psi_n$进行操作。

非常非常有意思的一点是，从GR的CHY被积函数出发，用这三个操作，以及非阿贝尔化，取单迹部分这些自然的物理想法，可以构造出一堆理论之间的联系：
