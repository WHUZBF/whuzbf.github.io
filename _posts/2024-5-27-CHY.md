...
- [CHY形式更详细的证明](#chy形式更详细的证明)
	- [n=3,n=4](#n3n4)
		...
		- [n=4](#n4)
			...
			- [最终结果](#最终结果)
	- [任意点](#任意点)
- [附录A：Kaluza-Klein理论](#附录akaluza-klein理论)
- [附录B：计算代数几何在CHY中的应用](#附录b计算代数几何在chy中的应用)
...
$$
\boxed{
I_{n}^{\mathrm{EYM}}(\mathrm{tr}_{1},\mathrm{tr}_{2},\ldots,\mathrm{tr}_{t};\mathrm{h}):=\left(\mathcal{C}_{\mathrm{tr}_{1}}\mathcal{C}_{\mathrm{tr}_{2}}\cdots\mathcal{C}_{\mathrm{tr}_{t}}\sum_{\{a,b\}}{}^{\prime}\mathcal{P}_{\{a,b\}}(\{\tilde{\epsilon}\})\right)\mathrm{Pf}^{\prime}\Psi_{n}(\{\epsilon\})}
$$

<p>注意除了外线动量以外还需要额外的输入，就是这里我们实际上计算的是那些$g$被引力子传播子分成$\mathrm{tr}_1\cup\mathrm{tr}_2\cup\cdots\cup\mathrm{tr}_t$的费曼图贡献的振幅（$\tr_i$内部排序需要模掉一个轮换，$\tr_i$之间排序无所谓），真正的振幅应该是所有的分法全部加起来得到的振幅。另外还注意，我们是从具有$2t$个光子的CHY被积函数出发进行非阿贝尔化的，但实际上这里并不是说是有$2t$个胶子散射，而是说考虑有$t$个trace的振幅，散射中涉及到的胶子个数要大于$2t$，如果等于$2t$那就说明每个trace里面只有两个$T^a$，也就是只取了abel部分。然后注意这里$\mathcal{P}$里面的$\Psi$，还是只取g指标集里面的$a_1,b_1,\ldots,a_{t-1},b_{t-1}$这$2t-2$个指标，当然，由于前面有求和，所以实际上所有胶子指标都参与运算了（除了最后一个没多大关系）。</p>
...
主要参考文献arXiv:1509.04483[^24]。CHY形式出来之后围绕的技术分两波，一拨人研究怎么快速计算CHY被积函数，进而从中提取出有用信息，另外一拨人研究怎么高效计算散射方程。首先要明确，散射方程的一般解析解不可能找到，因为都是高次方程了，伽罗瓦告诉我们没一般求解公式了。

比如对于五点，散射方程在$z_1=\infty,z_2=1,z_5=0$的规范固定下等价于：[^25]
...
$$
\begin{aligned}
0=h_m\equiv\sum_{S\in A,|S|=m}k_S^2z_S,\quad2\leq m\leq n-2
\end{aligned}
$$
...
$$
\begin{aligned}
\mathrm{GB}(I)=\left\langle s_{12}s_{45}+s_{12}s_{25}z_4-s_{13}s_{35}z_4+s_{14}s_{45}z_4+s_{14}s_{25}z_4^2,\right. \\
s_{12}+s_{13}z_3+s_{14}z_4\quad,\quad s_{45}z_3+s_{35}z_4+s_{25}z_3z_4\rangle
\end{aligned}
$$
...
T_{z_3}=
\begin{pmatrix}
-\frac{s_{12}}{s_{13}} & -\frac{s_{14}}{s_{13}} \\
\frac{s_{12}s_{45}}{s_{13}s_{25}} & \frac{s_{14}s_{45}-s_{13}s_{35}}{s_{13}s_{25}}
\end{pmatrix}\quad,\quad T_{z_4}=
\begin{pmatrix}
0 & 1 \\
-\frac{s_{12}s_{45}}{s_{14}s_{25}} & \frac{s_{13}s_{35}-s_{14}s_{45}-s_{12}s_{25}}{s_{14}s_{25}}
\end{pmatrix}
$$
...
$$
\mathcal{A}_5=\sum_{sol}\frac{z_{12}^2z_{25}^2z_{51}^2}{|\Phi|_{125}^{125}}\frac{1}{z_{12}^2z_{23}^2z_{34}^2z_{45}^2z_{51}^2}=\sum_{sol}\frac{1}{|\Phi|_{125}^{125}(z_3-1)^2(z_3-z_4)^2z_4^2}\equiv\sum_{z_3,z_4\in sol}\frac{P(z_3,z_4)}{Q(z_3,z_4)}
$$
...
\begin{aligned}
 ...
 & \text{Q} & & 
\begin{aligned}
 & = \Big(s_{35}(z_3-1)^2+s_{23}z_3^2\Big)(z_3-z_4)^2\Big(s_{45}(z_4-1)^2+s_{24}z_4^2\Big)
\end{aligned} \\
 & & & +s_{34}\bigg[s_{45}(z_3-1)^2z_3^2(z_4-1)^2 \\
 & & & +z_4^2\Big(z_3^2\Big(s_{24}(z_3-1)^2+s_{23}(z_4-1)^2\Big)+s_{35}(z_3-1)^2(z_4-1)^2\Big)\Big]
\end{aligned}
$$
...
$$
\begin{aligned}
sol_1:\quad z_3 & =\frac{-s_{12}s_{25}-s_{13}s_{35}+s_{14}s_{45}-\sqrt{\Delta}}{2s_{13}s_{25}}\quad,\quad z_4=\frac{-s_{12}s_{25}+s_{13}s_{35}-s_{14}s_{45}+\sqrt{\Delta}}{2s_{14}s_{25}} \\
sol_2:\quad z_3 & =\frac{-s_{12}s_{25}-s_{13}s_{35}+s_{14}s_{45}+\sqrt{\Delta}}{2s_{13}s_{25}}\quad,\quad z_4=\frac{-s_{12}s_{25}+s_{13}s_{35}-s_{14}s_{45}-\sqrt{\Delta}}{2s_{14}s_{25}}
\end{aligned}
$$
...
$$
\begin{aligned}
\Delta=(s_{12}s_{25}+s_{13}s_{35}-s_{14}s_{45})^2-4s_{12}s_{13}s_{25}s_{35}
\end{aligned}
$$
...
[^7]: 这将（可能）是我的另一篇文章
[^8]: 可见袁老师的[这篇博客](https://ellisyeyuan.wordpress.com/wp-content/uploads/2013/07/pfaffian_softlimits_factorizations_v2.pdf)
[^9]: DOI: 10.1007/JHEP07(2014)033；胡畅. 标量场一圈图CHY构造及散射方程解的研究[D].浙江大学,2022.
[^10]: 当然你可以去扯那些严格的数学
[^11]: arXiv:1010.3991
[^12]: arXiv:1311.5200
[^13]: 这部分具体怎么来的还没搞太懂
...
[^20]: 注意这两个外腿的顺序在$n$和$n-1$上有差别
[^21]: 为什么还倒过来引用后面的了？因为我是在CHY写完很久之后才意识到我对YMS理论其实理解不透彻。
[^22]: https://arxiv.org/abs/2401.00041
[^23]: https://arxiv.org/abs/1311.5200
[^24]: 如果想了解更多有关计算代数几何的技术细节，可以看网站download那一栏我的手写笔记。这方面最好的课自然是中科大张扬老师的相关课程。
[^25]: arXiv:1311.5200