...
首先我们要解决的问题是三个，但是他们之间有联系，首先就是不可重整定理告诉你SYM的最重要的结论就是beta函数one-loop exact。但是大家去真正计算物理相互作用的coupling constant的跑动[^8]的时候发现并非如此，也就是著名的NSVZ beta函数，最早的推导是用instanton的想法来的，依旧存在高圈贡献，而且众所周知beta函数本身是依赖于你选取的scheme的，而NSVZ beta函数没有告诉你scheme是啥。[^18]
...
最近听到几位同行求职并不顺利，即使研究方向热门、成果很好，也未必能拿到理想位置。哎，只能说做高能，天才很多，真的是如果没有超人智力能做的只有坚持到底，熬下去，最后熬出头。我显然是智力来讲差不少，比我聪明的同时期学生我见到的听到的过多，所以只能花时间想办法在大佬堆里面挤出一点生存空间。
...
另外今日还认识了一个Yuji的M2学生Ibo，以及Nakajima竟然还来IPMU办公，而不是仅仅挂职。我目前仍怀疑knot-quiver对偶和dimer model可积系统的直接物理含义。对数学物理工作而言，我个人更看重它是否能反过来解释或计算明确的物理量。
...
[^12]: 讨论中一度混淆了叠加态与混合态。在量子力学里面$\psi+\phi$还是纯态，混合态和叠加态一定要分开，叠加态是两个在一个希尔伯特空间的态加起来，混合态实际上两个态是在不同希尔伯特空间的，他们只是存在于一个统计系综里面，所以你实际上要用外直和，而体现到密度矩阵这边就是加法。
